...
export const createMicGainProcessor = (
  ...
  let cachedAudioContext: AudioContext | undefined;

  const build = (opts: AudioProcessorOptions) => {
    sourceTrack = opts.track;
    sourceTrack.applyConstraints({ noiseSuppression }).catch(() => {});
    const ctx = opts.audioContext ?? cachedAudioContext;
    if (!ctx) throw new Error('mic-gain processor: no AudioContext available');
    cachedAudioContext = ctx;
