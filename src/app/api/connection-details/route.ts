...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const roomPassword = params.get('password') ?? undefined;
    ...
    if (session) {
      ...
      if (!roomActive) {
        return Response.json({ error: 'guest_cannot_create' }, { status: 403 });
      }
      role = 'guest';
      login = null;
    ...
    if (!access.ok) {
      ...
      const denied = Response.json({ error: access.reason }, { status });
      if (session && admin !== session.isAdmin) await setSessionCookie(denied.headers, session.login, admin);
      return denied;
    }
    ...
    if (session && admin !== session.isAdmin) await setSessionCookie(response.headers, session.login, admin);
    await setRoomTokenCookie(response.headers, roomName, identity);
    return response;
  } catch (error) {
