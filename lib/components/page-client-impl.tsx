...
import { DebugMode } from '@/lib/components/debug';
import { KeyboardShortcuts } from '@/lib/components/keyboard-shortcuts';
import { RecordingIndicator } from '@/lib/components/recording-indicator';
import { SettingsMenu } from '@/lib/components/settings-menu';
...
import { useSetupE2EE } from '@/lib/use-setup-E2EE';
import { useLowCPUOptimizer } from '@/lib/use-perfomance-optimiser';
import { useWakeLock } from '@/lib/use-wake-lock';
import { MobileScreenShare } from '@/lib/components/mobile-screen-share';
import { Conference } from '@/lib/components/conference';
import { PreJoin } from '@/lib/components/pre-join';
...
export function PageClientImpl(props: {
  ...
  const preJoinDefaults = React.useMemo(() => {
    return {
      username: props.initialNickname ?? '',
      videoEnabled: true,
      audioEnabled: true,
    };
  ...
  const handlePreJoinSubmit = React.useCallback(
    async (values: LocalUserChoices) => {
      ...
      if (props.requiresPassword && roomPassword) {
    ...
    [props.roomName, props.region, props.requiresPassword, roomPassword, t],
  );
